...
public:
	...
	* @brief bin getter
	*/
	bool getBin(size_t hashKey, bin_t& bin) const;
	...
	*/
	bool allocTable(
		collision_t* collision//!< [in] collision list of the hash table
		);
...
* @brief collision getter
*/
inline collision_t HashTable::getCollision(
	size_t hashKey //!< [in] hash value
	) const
{
...
* @brief bin getter
*/
inline bool HashTable::getBin(size_t hashKey, bin_t& bin) const
{
	address_t table_p = hashTable[hashKey];
	if (!table_p)
	{
